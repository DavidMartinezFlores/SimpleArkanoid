...
def isCollisionWithBlock(score,multiplier,stick_width,bally_Speed,ball_radio,i,lifes,winnerValue,arrayOfBlocks,localizarPelota,block_width,block_height,ballx_Speed,ball_color):
    if (lifes>0 and winnerValue==0 and(arrayOfBlocks[i][0] < localizarPelota[0]+ball_radio < arrayOfBlocks[i][0] + block_width) and (arrayOfBlocks[i][1] < localizarPelota[1]+ball_radio < arrayOfBlocks[i][1] + block_height)):
