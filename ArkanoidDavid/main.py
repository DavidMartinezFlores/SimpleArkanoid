...
async def main():
    ...
    def printOnlyWinnerText():
        pygame.font.init()
        winnerA="!HAS GANADO LA PARTIDA! , AUTO-WIN CHEAT"
        font=pygame.font.SysFont('Comic Sans MS', 30)
        winnerAText=font.render(winnerA,True,(255,255,255))
        winnerARect=winnerAText.get_rect()
        winnerARect.center=(screen_width/2,screen_height/2)
        screen.blit(winnerAText,winnerARect)
        
    def autoWin(winnerValue,autoWinner):
        keyPressed = pygame.key.get_pressed()
        ...
        return winnerValue,autoWinner

    def bgHack(bg_img,secretBg):
        keyPressed = pygame.key.get_pressed()
        if keyPressed[pygame.K_DOWN]:
            if keyPressed[pygame.K_c]:
                if keyPressed[pygame.K_b]:
                    ...
                    if(secretBg>3):
                        secretBg=1
                    fondo="fondo"+str(secretBg)+".jpg"
                    bg_img = pygame.image.load(fondo)
                    bg_img = pygame.transform.scale(bg_img,(screen_width,screen_height))
                    
                    #Espera 1 sec
                    time.sleep(1)
        return bg_img,secretBg
    ...
    def ballStickCollision(ballx_Speed,ball_y,ball_x,ball_radio,stick_x,stick_width,Point,bally_Speed,ball_color,stick_height):
        ...
        if (ball_x+ball_radio >stick_x and ball_x-ball_radio < stick_x+stick_width and ball_y+ball_radio >= stick_y and ball_y+ball_radio <= stick_y+stick_height ):
            ...
            #Condicionales para evitar que descuadre la velociad al golpear un bloque de velociad x2 y rebotar en stick
            if(bally_Speed==8 or bally_Speed==-8):
                bally_Speed=-8
            else:
                bally_Speed=-4
    ...
    def resetGame(autoWinner,score,multiplier,stick_width,bally_Speed,ballx_Speed,ball_x,ball_y,ball_radio,lifes,winnerValue,arrayOfBlocks,arrayOfBlocksReset):
        lifes=3
        winnerValue=0
        ...
        return autoWinner,score,multiplier,stick_width,bally_Speed,ballx_Speed,ball_x,ball_y,ball_radio,lifes,winnerValue,arrayOfBlocks,arrayOfBlocksReset
    ...
    def isCollisionWithBlock(score,multiplier,stick_width,bally_Speed,ball_radio,i,lifes,winnerValue,arrayOfBlocks,localizarPelota,block_width,block_height,ballx_Speed,ball_color):
        if (lifes>0 and winnerValue==0 and((arrayOfBlocks[i][0]-ball_radio < localizarPelota[0]-ball_radio < arrayOfBlocks[i][0] + block_width)or(arrayOfBlocks[i][0]-ball_radio < localizarPelota[0]+ball_radio < arrayOfBlocks[i][0] + block_width)) and (arrayOfBlocks[i][1] < localizarPelota[1]-ball_radio < arrayOfBlocks[i][1] + block_height or (arrayOfBlocks[i][1] < localizarPelota[1]+ball_radio < arrayOfBlocks[i][1] + block_height))):
            print("Tocado un bloque en la coordena ",arrayOfBlocks[i]," bloque numero ",str(i))
            arrayOfBlocks[i]=(999999,999999,999999,999999)
            ballx_Speed = ballx_Speed #Por si se quiere invertir X
            bally_Speed *=(-1)
            ball_color=(0,0,255)
            score+=multiplier
    ...
    mixer.music.play(-1)#Musica en bucle por el -1
    pygame.mixer.music.set_volume(0.5)
    ...
    a = pygame.image.load('favicon.png')
    ...
    while running:
        ...
        if keyPressed[pygame.K_u] and (lifes <=0 or winnerValue==1): # Reset Partida
            stick_x,stick_y,ball_x,ball_y,Point,ballx_Speed,bally_Speed=resetLocations(ball_x,ball_y,ball_radio,screen_height,stick_x,Point,ballx_Speed,bally_Speed)
            autoWinner,score,multiplier,stick_width,bally_Speed,ballx_Speed,ball_x,ball_y,ball_radio,lifes,winnerValue,arrayOfBlocks,arrayOfBlocksReset=resetGame(autoWinner,score,multiplier,stick_width,bally_Speed,ballx_Speed,ball_x,ball_y,ball_radio,lifes,winnerValue,arrayOfBlocks,arrayOfBlocksReset)
        ...
        if(winnerValue==1):
            ...
            if(autoWinner==1):
                printOnlyWinnerText()#CREADO PARA IMPRIMIR SOLO EL TEXTO , PARA EL AUTOWIN
        ...
        #CHEATS / HACKS
        lifes=lifeHack(lifes) 
        stick_width=stickHack(stick_width)
        bg_img,secretBg=bgHack(bg_img,secretBg)
        winnerValue,autoWinner=autoWin(winnerValue,autoWinner)
        ...
        #-----------GAME-WHILE-END---------------------------
        await asyncio.sleep(0)  # Very important put on 0 and stay inside of your principal loop.
